...
package worldmap.terasology.someClasses;
...
public final class TeraMath {
	...
	/**
	 * a + b, but if the result exceeds Integer.MAX_VALUE then the result will
	 * be Integer.MAX_VALUE rather than overflowing.
	 *
	 * @param a
	 * @param b
	...
	public static float pow(float base, int exp) {
		if (exp <= 0) {
			if (exp == 0) {
				// 0^0 is an indetermination, and should therefore return NaN,
				// but Java Math.pow does return 1.0,
	...
	/**
	 * Modulus operation, where the result has the same sign as the divisor. <br>
	 * <br>
	 ...
	 *
	 * @param dividend
	 *            The value that is divided
	 * @param divisor
	 *            The value with which is divided
	 * @return The remainder of (dividend / divisor) as a number in the range
	 *         [0, divisor)
	 * @author DizzyDragon
	 */
	...
	/**
	 * Modulus operation, where the result has the same sign as the dividend. <br>
	 * <br>
	 * Modulus(a, b) equals a % b. This function (alias) exists primarily to be
	 * used in places where both modulus and % are used, to make a clearer
	 * distinction between the two operations.
	 *
	 * @param dividend
	 *            The value that is divided
	 * @param divisor
	...
	/**
	 * Lowest power of two greater or equal to val <br>
	 * <br>
	 * For values &lt;= 0 returns 0 For values &gt;= 2 ^ 30 returns 0. (2^30 is
	 * the largest power of 2 that fits within a int).
	 *
	 * @param val
	 ...
	 * 
	 * @param array
	 *            an array with equal size in both dimensions
	...
	/**
	 * Populates a target array with the minimum value adjacent to each
	 * location, including the location itself.
	 *
	 * @param source
	 * @param target
	 * @param populateMargins
